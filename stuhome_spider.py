...
class StuhomeSpider():
    ...
    def reply(self,fid,tid,extra,formhash):
        ...
        # try:
        #     r = self.session.post(url=self.reply_url.format(fid,tid,extra),data=post_data,headeers=self.headers)
        #     print('回复成功！')
        # except :
        #     print('回复失败，请重试!')
    ...
    def run(self):
        ...
        if login_result:
            self.tiezi_url = input('登录成功!请输入要回复的帖子url：')
            self.get_tiezi_params_and_reply()
